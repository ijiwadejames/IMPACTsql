...
import PersonalCommentBox from "./PersonalCommentBox";
import PersonalEditPost from "./PersonalEditPost";
import PersonalUnlikeButton from "./PersonalUnlikeButton";
import PersonalLikeButton from "./PersonalLikeButton";
import PersonalDeleteButton from "./PersonalDeleteButton";
import { postEngagement } from "./features/engagement/engagementSlice";
import he from "he";

const PersonalPostStatusBar = ({ post }) => {
  ...
  return (
    <>
      {edit === true && (
        <>
          <PersonalEditPost key={post.id} post={post} />
        </>
      )}
      <div className="status-bar-items">
        <div className="items">
          {post.likedBy?.some((likes) => likes.userID === (user && user.id)) ? (
            <>
              <PersonalUnlikeButton key={post.id} post={post} />
            </>
          ) : (
            <>
              <PersonalLikeButton key={post.id} post={post} />
            </>
          )}
        </div>
        <div className="items">
      ...
      <div className="p-0 m-0">
        {toggleDiscussion && (
          <div className="col-11 mt-3 m-auto">
            <div className="col-12 fw-bold">Join Discussion</div>
            ...
            <div>
              <PersonalCommentBox key={post.id} post={post} />
            </div>
          </div>
        )}
      </div>
...
export default PersonalPostStatusBar;
