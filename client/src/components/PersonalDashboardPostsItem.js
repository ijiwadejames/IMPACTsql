...
import PersonalPostStatusBar from "./PersonalPostStatusBar";
import {
  faCheckCircle,
  faUserCircle,
  faUserClock,
...
const PersonalDashboardPostsItem = (props) => {
  ...
  return (
    ...
    >
        ...
        <div>
          {triggerDrop === true && (
            <>
              ...
              <PersonalDeleteButton color="black" key={post.id} post={post} />
              <button
                className="btn-bg border-0 p-0 m-1 font-twelve"
      ...
      <PersonalPostStatusBar key={post.id} post={post} />
    </div>
  );
};

export default memo(PersonalDashboardPostsItem);
